...
```bash
cd "/root/VPN Bots/Sulsocratus_bot"
python3 -m venv venv
...
```bash
python3 "/root/VPN Bots/scripts/apply-xui-panel-settings.py" Sulsocratus_bot.db
```
...
# یا:
tmux new-session -d -s Sulsocratus_bot -c "/root/VPN Bots/Sulsocratus_bot"
tmux send-keys -t Sulsocratus_bot 'source venv/bin/activate && python3 main.py' Enter
```
